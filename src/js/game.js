import { WORDS, KEYBORD_LETTERS } from "./consts";
...
const createPlaceholdersHTML = () => {
  ...
  //2
  // const signArray = Array.from('_'.repeat(word.length));
  // const placeholdersHTML = signArray.reduce(
  // (acc, cur, i) => acc + `<h1 id="letter_${i}" class="letter"> _ </h1>`, '');
...
const createKeyboard = () => {
  ...
  const keyboardHTML = KEYBORD_LETTERS.reduce((acc, cur) => {
    return (
      acc +
...
const checkLetter = (letter) => {
  ...
  if (!word.includes(inputLetter)) {
    ...
    trieLeft -= 1;
    triesCounter.innerText = trieLeft;

    const hangmanImg = document.getElementById("hangman-img");
    hangmanImg.src = `images/hg-${10 - trieLeft}.png`;
...
export const startGame = () => {
  ...
  gameDiv.insertAdjacentHTML('beforeend', '<button id="quit" class="butten-secondary px-2 py-1 mt-4">Quit</button>')
